...
CCShake::CCShake()
    : m_strength_x(0), m_strength_y(0), m_initial_x(0), m_initial_y(0) {}
CCShake *CCShake::create(float d, float strength) {
  // call other construction method with twice the same strength
  return createWithStrength(d, strength, strength);
}
CCShake *CCShake::createWithStrength(float duration, float strength_x,
                                     float strength_y) {
...
bool CCShake::initWithDuration(float duration, float strength_x,
                               float strength_y) {
...
void CCShake::update(float dt) {
  ...
  float randy = fgRangeRand(-m_strength_y, m_strength_y) * dt * 0;
  // move the target to a shaked position
  _target->setPosition(
      ccpAdd(ccp(m_initial_x, m_initial_y), ccp(randx, randy)));
}
...
CCFallOffShake *CCFallOffShake::createWithStrength(float duration,
                                                   float strength_x,
                                                   float strength_y) {
